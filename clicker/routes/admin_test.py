...
from ..utils.reverse_name import reverse_name
...
def enable_test():
    if request.method == 'POST':
        ...
        if bool(enable_test):
            ...
            set_enable_test_status(False)

        set_end_test_status(False)
        set_auth_see_sol_status(False)
        set_grade_already_status(False)
...
def view_test():
    ...
    if get_enable_test_status():
        return render_template("test.html", id=id, quiz=quiz_, quiz_ordering=json.dumps(ordering))
    else:
        return render_template("index.html")


@admin_test.route('/check_test/<id>', methods=['GET', 'POST'])
@student_permission.require(http_exception=403)
@login_required
def check_test(id):
    if request.method == 'POST':

        if not get_end_test_status():
...
def view_test_sol():
    if request.method == 'GET':
        ...
        user = User.query.filter_by(id=current_user.id).first()
        if current_user.is_active and current_user.sent_answers and get_auth_see_sol_status() and get_end_test_status() and \
                get_grade_already_status() and current_user.id == user.id:
    ...
    return redirect(url_for('admin_test.view_test_sol'))
...
def show_statistics():
    if get_grade_already_status():
        ...
        return render_template('chart.html',
                               ...
                               title2='GRADE DISTRIBUTION, TEST IS WORTH ' + str(
                                   len(binx) - 1) + ' POINTS AND EACH QUESTION IS WORTH 1 POINT',
...
def save_results_pdf():
    if not get_grade_already_status():
        ...
        if user:
            ...
            for i in user:
                # The next line of code transform Luis Alejandro Ladino Gaspar into Ladino Gaspar Luis Alejandro

                full_name = reverse_name(i.name)
...
def new_test():
    if request.method == 'POST':
        ...
        if new_test:
            return jsonify({'message': "A new test has started. Go to the Test Bank and set up the test.",
                            'enable': "Yes"})
        else:
...
@admin_test.errorhandler(403)
def authorisation_failed(e):
    #flash(('Your current identity is {id}. You need special privileges to'
          # ' access this page').format(id=g.identity.name))

    #return render_template('Unauthorized.html', priv=current_privileges())
    return render_template('unauthorized.html',  message='You need special privileges to access this page')

def set_commands_table():
    set_grade_already_status(False)
    set_auth_see_sol_status(False)
    set_end_test_status(False)
    set_enable_test_status(False)
